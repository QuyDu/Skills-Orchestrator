...
1. Create the project by running, from the repository root:
   ...
   The project is created at `C:\repos\skills-orchestrator-demo`. Stop and report if that path already exists; do not delete it.
...
4. Copy `.github/prompts/demo-web-app.prompt.md` from this repository into the created project at the same relative path, so the presenter can run `/demo-web-app` there without pasting prompt text.
