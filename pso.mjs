...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (["help", "version", "dry-run", "apply", "accept-risk", "force-templates", "force-adopt"].includes(key)) {
      result[key] = true;
      continue;
    }
...
async function createProject({ name: enteredName, destination, profile = "core", stack = "", riskAcceptance }) {
  ...
  try {
    ...
    const files = new Map([
      ["README.md", `# ${displayName}\n\nProvisioned with Project Skills Orchestrator ${VERSION} using the ${profile} profile${declaredStack.size ? ` for the ${[...declaredStack].sort().join(", ")} stack` : ""}.\n\n## What this repository currently contains\n\nThis is a governed development baseline: agent instructions, scoped standards, reusable prompts, specialist agents, the skill catalog, and a continuous-integration workflow. **It does not yet contain application code.** Add your application under \`src/\` and its tests under \`tests/\`.\n\n## First steps\n\n1. Open \`${name}.code-workspace\` in Visual Studio Code.\n2. Read \`.github/copilot-instructions.md\`. Every agent prompt in this project begins with the mandatory clarification protocol defined there.\n3. In GitHub Copilot Chat Agent mode, run \`/development-environment-readiness\` to validate tools, runtimes, authentication, debugging, and security gates.\n4. Run \`/project-setup\` if you want the agent to review this baseline against the application you are about to build.\n\n${declaredStack.size ? "The continuous-integration workflow runs real build and test commands for the declared stack. Confirm they match this project before relying on the result." : "The continuous-integration workflow fails until you configure a build and test command in `.github/workflows/ci.yml`. That is deliberate: a pipeline that passes without testing anything is worse than no pipeline."}\n`],
      ...
      [".vscode/extensions.json", `${JSON.stringify({ recommendations: ["github.copilot", "github.copilot-chat"] }, null, 2)}\n`],
      [".vscode/settings.json", `${JSON.stringify({ "files.trimTrailingWhitespace": true, "editor.formatOnSave": true, "chat.promptFiles": true, "github.copilot.chat.codeGeneration.useInstructionFiles": true }, null, 2)}\n`],
  ...
  return { name, root, workspace: path.join(root, `${name}.code-workspace`), profile };
}
...
async function buildAdoptionPlan(projectRoot, profileOverride, projectNameOverride, { forceTemplates = false, forceAdopt = false } = {}) {
  ...
  const generated = [
    ...
    },
    {
      ...
      content: `${JSON.stringify({ recommendations: ["github.copilot", "github.copilot-chat"] }, null, 2)}\n`,
      preserveExisting: true
    },
    {
      path: ".vscode/settings.json",
      kind: "workspace-support",
      content: `${JSON.stringify({ "files.trimTrailingWhitespace": true, "editor.formatOnSave": true }, null, 2)}\n`,
      preserveExisting: true
    }
  ];
...
async function guidedCreate() {
  ...
  try {
    ...
    const result = await createProject({ name, destination: destination.trim() || process.cwd(), riskAcceptance });
...
Usage:
  node .\\pso.mjs
  node .\\pso.mjs create-project --name "My Project" --destination "C:\\repos" --profile core --accept-risk
...
async function main() {
  ...
  if (command === "create-project") {
    ...
    console.log(`Created project: ${result.root}`);
    return console.log(`Open workspace: ${result.workspace}`);
  }
